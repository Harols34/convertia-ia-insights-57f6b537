...
import { Brain, Loader2, Sparkles, Save, History } from "lucide-react";
...
const SUGGESTIONS = [
  ...
  "¿Cuáles son las campañas con mayor tasa de conversión?",
  "Analiza la distribución de leads por BPO y resultado de negocio",
  "¿Cuáles son las ciudades con más leads?",
  "Compara el rendimiento de los agentes de primera gestión",
];
...
export default function DashboardsIAPage() {
  ...
  const handleSend = (text: string) => {
    sendMessage(text, { mode: "analytics" });
    setPrompt("");
  };
    ...
    <div className="space-y-6">
      ...
      {messages.length === 0 && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
          <div className="rounded-xl border border-border bg-card p-6 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-4">Escribe en lenguaje natural qué métricas o análisis necesitas</p>
            <div className="flex flex-wrap gap-2 justify-center">
              {SUGGESTIONS.map((s) => (
                <Button key={s} variant="secondary" size="sm" className="text-xs" onClick={() => handleSend(s)}>
                  <Sparkles className="h-3 w-3 mr-1" /> {s.slice(0, 50)}{s.length > 50 ? "…" : ""}
                </Button>
              ))}
            </div>
          </div>
      ...
      <div className="h-[550px]">
        <ChatWindow
          ...
          isLoading={isLoading}
          placeholder="Escribe qué métricas deseas ver..."
        />
      </div>
    </div>
  );
